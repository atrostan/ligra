...
struct CF_Edge_F {
  ...
  vertex *V;
  int K;
  CF_Edge_F(vertex *_V, double *_latent_curr, double *_error, int _K) : V(_V), latent_curr(_latent_curr), error(_error), K(_K) {}
  inline bool update(uintE s, uintE d, intE edgeLen) {//updates latent vector based on neighbors' data
    // std::cout << s << " " << d << " " << edgeLen << "\n";
    double estimate = 0;
...
struct CF_Vertex_F {
  ...
  double *latent_curr, *error;
  int K;
  CF_Vertex_F(double _step, double _lambda, double *_latent_curr, double *_error, int _K) : step(_step), lambda(_lambda), latent_curr(_latent_curr), error(_error), K(_K) {}
